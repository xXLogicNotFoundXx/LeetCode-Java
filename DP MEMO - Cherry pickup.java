...
We can have two people starting from 0,0 and collect maximum cherries.
...
So having cherry pick up by person1 and then person2 wont give the correct result. 
...
But, with our approach person1 will collect 9 cherries leaving once that are on the right(1,4) and on the left(2,0). 
Then person2 wont be able to collect both cherries he can collect only right one or only left one. 

Approach #2: 
We want collectively maximum cherries.
So we know that we have to do the traversal of both persons at the same time and select maximum global ans. 
...
Following code is simple backtracking brute force so it is TLE 
I think Time Complexity is : 4^N*N. As we are calling cherryPickup 4 times recursively with problem size N*N.

Reference: Thanks @kai99 for this awesome backtracking solution. 
...
dp[r1][c1][r2][c2] will give identify each the state. 
So if dp[r1][c1][r2][c2] is null then that means we haven't computed subanswer for that state. 
