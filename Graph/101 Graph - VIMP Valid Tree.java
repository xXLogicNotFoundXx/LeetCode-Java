...
Graph Valid Tree - bi-directional graph :
valid tree is :
1 you visit all nodes in the graph starting from any node
2 there is no cycle.
...
class Solution {
    /*
    Approach two :
        Graph Theroy - For the graph to be a valid tree,
          1 it must have EXACTLY n - 1 edges
          2 all nodes must be visited when you traverse the graph.
    ...
    */
    public boolean validTree(int n, int[][] edges) {
        ...
        if(edges.length != n-1)   // edges n-1. Assumptions -  1 No self loop edges. 2 No duplicate edges. 3 No invalid node edges.
            return false;
...
class Solution {
    ...
    Approach Three :
        ...
        IF graph has n - 1 edges and no cycle then it is a valid tree. (Assumption - edges does not have duplicate data)
