...
class Solution1 {
    ...
    // memory limit exceed bcz we create a grid of NxN where N=10000 and all these 4 arrays with N
    ...
    boolean isLit(int x, int y, int cN, int[] row,int[] col,int[] diag,int[]backSlash){
        return row[x]>0 || col[y]>0 || diag[x+y]>0 || backSlash[y-x+cN]>0;
    }
}

// This one is nice solution. Using minimal memory. Instead of Creating [NxN] grid to store location of the lamp
// we store locations of the lamp in a Set (key = "x-y") .. (assuming duplicate lamps are not in the array lamps .. else use hashmap)
