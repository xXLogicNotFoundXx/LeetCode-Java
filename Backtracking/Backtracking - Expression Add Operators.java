...
public class Solution {
    ...
    public void helper(List<String> rst, String path, String num, int target, int pos, long eval, long prev){
        if(pos == num.length()){
            if(target == eval)
                rst.add(path);
            return;
        ...
        for(int i = pos; i < num.length(); i++){
            ...
            if(pos == 0){
                ...
                helper(rst, path + "*" + cur, num, target, i + 1, (eval - prev) + (prev * cur), prev * cur );
            }
