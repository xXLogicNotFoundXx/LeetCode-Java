...
Person #0 gave person #1 $10.         0-> +10 and 1 -> -10
Person #1 gave person #0 $1.          1-> -9 and  0 -> 9
Person #1 gave person #2 $5.          1-> -4 and  2 -> -5
Person #2 gave person #0 $5.          2->  0 and  0 -> 4
Map :
0 -> 4
1 -> -4
2 -> 0
...
and internal loop from i=start+1 to N
    ...
    Observation ->
    giving all money was a good adea
    Two people having +ve&+ve or -ve&-ve then we should not do the transaction bcz it will increase the count of transasctions.
*/
class Solution {
    ...
    int settle(int start, List<Integer> debt) {
        ...
        for (int i = start + 1; i < debt.size(); i++){
            // one should be +ve and one should be -ve
            // ( -ve&-ve and +ve &+ve no need of transaction)
            if (debt.get(start) * debt.get(i) < 0) {

                int oldIVal = debt.get(i);
                int newIVal = debt.get(i) + debt.get(start); // very IMPORTANT ... here start is giving all his money to i
